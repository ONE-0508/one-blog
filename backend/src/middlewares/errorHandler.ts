...
const ERROR_MESSAGE_MAP: Record<string, string> = {
  ...
  'Tag ids must be an array': '标签 ID 必须是数组',
  'Author is required': '作者信息不能为空',
  'No fields to update': '缺少可更新字段',
  'Tag id is required': '标签 ID 不能为空',
  'Tag not found': '标签不存在',
  'Tag name is required': '标签名称不能为空',
  'Tag name must be 30 characters or less': '标签名称长度不能超过 30 字',
  'Tag slug is required': '标签 slug 不能为空',
  'Tag slug must use lowercase letters, numbers, and hyphens':
    '标签 slug 只能包含小写字母、数字和连字符',
  'Tag color is required': '标签颜色不能为空',
  'Tag color must be a valid HEX color': '标签颜色必须为合法 HEX 色值',
  'Tag status is invalid': '标签状态无效',
  'Tag slug already exists': '标签 slug 已存在',
  'Tag is invalid': '标签无效或已禁用',
  'Please remove this tag from related articles before deleting':
    '请先在相关文章中移除该标签后再删除',
